#!/usr/bin/env python3
"""
Wazobia Audio Server - REST API
Handles audio file uploads and returns agent responses
"""
...
# Create FastAPI app
app = FastAPI(title="Wazobia Audio Server")
...
class AudioFileClient:
    """Same class from audio_client.py"""
    ...
    async def connect_and_chat(self, input_file, output_file):
        ...
        print(f"\n{'='*60}")
        print(f"🎤 Processing Audio Request")
        print(f"{'='*60}")
        ...
        success = await self.save_response(output_file)
...
async def root():
    """Root endpoint"""
    return {
        "service": "Wazobia Audio Server",
        "status": "running",
        "endpoints": {
            "chat": "POST /chat - Upload audio file, get response",
...
async def health():
    ...
    return {"status": "healthy", "service": "wazobia-audio-server"}


@app.post("/chat")
async def chat(audio: UploadFile = File(...)):
    """
    Upload audio file and get agent response
    
    Args:
        audio: Audio file (mp3, wav, m4a, etc.)
        
    Returns:
        Audio file with agent response
    """
    print(f"\n{'='*60}")
    ...
    # Save uploaded file
    input_filename = f"{request_id}_input{Path(audio.filename).suffix}"
    ...
    output_path = RESPONSE_DIR / output_filename
    
    # Process audio
    try:
        client = AudioFileClient(room_name=f"server-{request_id}")
        success = await client.connect_and_chat(str(input_path), str(output_path))
        ...
        return FileResponse(
            ...
            filename=f"response_{request_id}.wav"
        )
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("🚀 Starting Wazobia Audio Server")
    print("="*60)
    print(f"Server will run on: http://localhost:8000")
    print(f"Endpoints:")
    print(f"  - POST /chat     (upload audio, get response)")
